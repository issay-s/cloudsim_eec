...
private:
    ...
    static constexpr unsigned kIdleChecksBeforeSleep = 2;
    static constexpr unsigned kDeepSleepChecks = 6;
    static constexpr unsigned kMinAwakePerCPU = 1;
    ...
    bool TryPlaceTask(TaskId_t task_id)
    {
        ...
        for (MachineId_t id = 0; id < machine_count; id++) {
            ...
            if (util < fallback_metric) {
                fallback_metric = util;
                fallback = id;
            }

            if (urgent) {
                // Urgent tasks prefer lower queueing delay.
                if (util <= kUpperUtilUrgent && util < best_metric) {
                    best_metric = util;
                    best = id;
                }
            } else {
                // Best-effort tasks prefer consolidation under the cap.
                if (util <= kUpperUtilBestEffort && util > best_metric) {
                    best_metric = util;
                    best = id;
                }
            }
    ...
    void SleepIdleMachines()
    {
        ...
        if (!pending_tasks.empty() || checks_since_new_task < kIdleChecksBeforeSleep) {
            return;
        }
